...
void clique::create_grid() {
    ...
    std::ofstream file;
    file.open("data_info_details.txt");

    for(auto it = info.m_max_corner.cbegin() ; it < info.m_max_corner.cend() ; it++)
    {
        file<<*it<<" ";
    }
    file<<"\n";
    for(auto it = info.m_min_corner.cbegin() ; it < info.m_min_corner.cend() ; it++)
    {
        file<<*it<<" ";
    }
    file<<"\n";
    for(auto it = info.m_sizes.cbegin() ; it < info.m_sizes.cend() ; it++)
    {
        file<<*it<<" ";
    }
